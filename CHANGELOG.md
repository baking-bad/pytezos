...
- `pytezos sandbox` starts a working chain again: `--protocol` lists the supported protocols (default: the latest), `get_protocol_parameters` honours its argument, the CLI retries a failed bake, and the node settles for a moment after protocol activation so the first mempool request cannot wedge the prevalidator.
...
- `forge_protocol_data` encoded every `liquidity_baking_toggle_vote` as `off` and ignored `adaptive_issuance_vote`. Votes are now typed (`pytezos.block.forge.PerBlockVote`) and forged per the protocol's per-block-votes encoding: a single on/off/pass byte since Jakarta, with the adaptive issuance vote packed into the same byte for Oxford..Seoul headers. Values outside `on`/`off`/`pass` raise `ValueError`.
- Contracts with an entrypoint or view named like a `ContractInterface` attribute (`storage`, `metadata`, `token_metadata`, `call`, ...), or with an entrypoint and a view sharing a name (e.g. FA2.1 `balance_of`), load again; every proxy is reachable via `contract.entrypoint.<name>` / `contract.view.<name>`, and a shadowed top-level shortcut emits a `UserWarning` instead of raising (#314).
- TZIP-21 token metadata: `ContractTokenMetadata.rightsUri` is now populated from either the spec key `rightsUri` or the legacy `rightUri` (previously the bundled schema only knew `rightUri`, so the field was always `None`). Unknown top-level keys no longer fail validation, matching `additionalProperties: true` in the TZIP-21 schema; they are kept in `.raw`, which holds the document exactly as given (a legacy `rightUri` stays there under its own name). Nested `formats[]`, `attributes[]`, `dimensions` and `dataRate` objects stay strict as in the spec.
